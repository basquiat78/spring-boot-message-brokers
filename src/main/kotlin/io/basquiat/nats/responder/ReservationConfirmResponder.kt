...
@Component
class ReservationConfirmResponder(
    natsConnection: Connection,
    props: NatsProperties,
    taskExecutor: TaskExecutor,
    private val usecase: ReservationActionHandler,
) : AbstractResponder<ReservationActionResponse>(natsConnection, props, taskExecutor, ApiSubject.RESERVATION_CONFIRM) {
    override fun handleRequest(msg: Message): ReservationActionResponse {
        ...
        return try {
            usecase.execute(request, ReservationActionType.CONFIRM)
        } catch (e: Exception) {
